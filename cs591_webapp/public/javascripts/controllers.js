angular.module('cs411', ['ngRoute', 'ngCookies'])
    ...
    .controller('cs411ctrl', function ($scope, $http, $cookies) {
        ...
        //show events
        $scope.show_event = function (location, event) {
            $http.get('http://localhost:3000/event/' + event + '/' + location)
                .then(function (response) {
                    // console.log(data.last_item);
                    $scope.events = response.data.events.event; //checkback
                })
        }
